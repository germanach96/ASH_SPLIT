"""La segunda parte del informe: la ficha de cada planner.

Este modulo no se ejecuta solo, lo llama build_pdf.py para juntar su cuerpo con
el de build_report.py en un unico documento.
...
def construir(d, shots):
    ...
    w('<div class="foot">Generated by <span class="mono">build_pdf.py</span> from '
      '<span class="mono">Ashford split 2.xlsx</span> and '
      '<span class="mono">ownership_proposal.json</span>. The maps are captures of '
      '<span class="mono">ashford_bom_graph_proposal.html</span> filtered per planner with the '
      'arrows removed. Every figure comes from the same split the viewer carries.</div>')

    return '<div class="planner">' + "".join(W) + "</div>"
